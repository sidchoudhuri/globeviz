...
def screen_codes(text):
    """ASCII -> screen codes for the lowercase charset."""
    out = bytearray()
    for ch in text:
        c = ord(ch)
        if ord("a") <= c <= ord("z"):
            out.append(c - 96)
        elif ord("A") <= c <= ord("Z") or ord(" ") <= c <= ord("?"):
            out.append(c)
        elif ch == "@":
            out.append(0)
        else:
...
def emit_asm(tex, rowhi, thbase, collut, charset_d018=0x17, sid=None, zp_ptr=0xFB,
             text=None, text_row=0, scroll=None, scroll_row=24, scroll_speed=3):
    blocks = "".join(render_block(n) for n in range(4))
    tex_bytes = b"".join(tex)
    tex_org = TEX_PAGE << 8

    text_init, ticker_call, text_block = "", "", ""
    if text:
        codes = screen_codes(text.center(40)[:40])
        text_init += f"""
        ldx #39         ; static text, row {text_row}
-       lda stext,x
        sta ${0x0400 + text_row * 40:04x},x
        lda #{TEXT_COLOR}
        sta ${0xd800 + text_row * 40:04x},x
        dex
        bpl -
"""
        text_block += f"""
...
"""
    if scroll:
        buf = screen_codes(scroll + "   ")
        buf += buf[:40]          # duplicated head -> seamless wrap, linear copy
        text_init += f"""
        ldx #39         ; scroll ticker color, row {scroll_row}
-       lda #{TEXT_COLOR}
        sta ${0xd800 + scroll_row * 40:04x},x
        dex
        bpl -
"""
        ticker_call = "        jsr ticker\n"
        text_block += f"""
ticker  dec tickcnt     ; char-scroll every {scroll_speed} render loops
        bne tdone
        lda #{scroll_speed}
        sta tickcnt
        ldy #39
tsrc    lda sbuf,y      ; self-modified base = sbuf + offset
        sta ${0x0400 + scroll_row * 40:04x},y
        dey
        bpl tsrc
        inc tsrc+1
        bne +
        inc tsrc+2
+       lda tsrc+1      ; wrapped past sbuf+len? reset to sbuf
        cmp #<sbufend
        lda tsrc+2
        sbc #>sbufend
        bcc tdone
...
tdone   rts
tickcnt .byte 1
sbuf                    ; "{scroll}" + separator, head duplicated
{byte_rows(buf)}
sbufend = sbuf + {len(buf) - 40}
"""
    if sid:
        music_init = f"""
...
def main():
    ...
    ap.add_argument("--text", default=None, help="static text line (max 40 chars, centered)")
    ap.add_argument("--text-row", type=int, default=0)
    ap.add_argument("--scroll", default=None, help="scrolling ticker text")
    ap.add_argument("--scroll-row", type=int, default=24)
    ap.add_argument("--scroll-speed", type=int, default=3,
                    help="render loops per 1-char scroll step (default 3)")
    ...
    for row, what in ((args.text_row, args.text), (args.scroll_row, args.scroll)):
        if what is not None:
            assert 0 <= row <= 24, "text row must be 0-24"
            if 1 <= row <= 23:
                print(f"WARNING: row {row} overlaps the globe (rows 1-23)")
    if args.text:
        assert len(args.text) <= 40, "static text exceeds 40 chars"

    asm = emit_asm(tex, rowhi, thbase, collut, sid=sid, zp_ptr=zp_ptr,
                   text=args.text, text_row=args.text_row,
                   scroll=args.scroll, scroll_row=args.scroll_row,
